...
import { 
  ...
  AppNotification, OpticalRx, FinancialGoal, Branch, ChatMessage
} from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
        ...
        if (!socketRef.current) {
            const socket = io({ transports: ['websocket', 'polling'], reconnection: true }) as any;
            ...
            socket.on('DATA_UPDATED', (payload: any) => {
                if (payload.actorId !== currentUser.id) {
                    ...
                    if (['settings', 'membership', 'family'].includes(payload.entity)) checkAuth();
                    else refreshData();
                }
            });
  ...
  const checkAuth = async () => {
    ...
    if (!token) {
        setState(null); setCurrentUser(null); setAuthChecked(true); setDataLoaded(true);
        return;
    }
    try {
        ...
        localStorage.removeItem('token'); setCurrentUser(null); setState(null);
    } finally {
        setAuthChecked(true); setDataLoaded(true);
    }
  };
  ...
  useEffect(() => {
    const initApp = async () => {
        ...
        if (!publicToken) await checkAuth();
        else { setAuthChecked(true); setDataLoaded(true); }
    };
    initApp();
  }, [publicToken]);
  ...
  const handleLoginSuccess = async (user: User) => {
      setDataLoaded(false); setState(null); setCurrentUser(user); setShowAuth(false);
      try {
          const data = await loadInitialData();
          setState(data);
      } finally {
  ...
  const safeState = useMemo(() => {
    ...
    const filterByFamily = (items: any[]) => (Array.isArray(items) ? items.filter(item => (item.familyId || item.family_id) === activeFamilyId) : []);
    return {
        ...
        companyProfile: state.companyProfile && (state.companyProfile.familyId === activeFamilyId || (state.companyProfile as any).family_id === activeFamilyId) ? state.companyProfile : null
    };
  }, [state, currentUser]);

  const renderContent = () => {
    if (!dataLoaded || !safeState || !currentUser) return <LoadingOverlay isVisible={true} />;
    ...
    switch (currentView) {
      ...
      case 'OPTICAL_RX':
          return <OpticalModule opticalRxs={safeState.opticalRxs} contacts={safeState.contacts} onAddRx={() => { setEditingRx(null); setCurrentView('OPTICAL_RX_EDITOR'); }} onEditRx={(rx) => { setEditingRx(rx); setCurrentView('OPTICAL_RX_EDITOR'); }} onDeleteRx={(id) => api.deleteOpticalRx(id).then(refreshData)} />;

      case 'OPTICAL_RX_EDITOR':
          return <OpticalRxEditor contacts={safeState.contacts} branches={safeState.branches} initialData={editingRx} onSave={(rx) => api.saveOpticalRx(rx).then(() => { refreshData(); setCurrentView('OPTICAL_RX'); })} onCancel={() => { setEditingRx(null); setCurrentView('OPTICAL_RX'); }} />;

      case 'ODONTO_AGENDA':
      case 'ODONTO_PATIENTS':
          return <ServiceModule {...commonProps} moduleTitle="Clínica Odontológica" clientLabel="Paciente" serviceLabel="Procedimento" transactionCategory="Serviços Odontológicos" activeSection={currentView === 'ODONTO_AGENDA' ? 'CALENDAR' : 'CLIENTS'} clients={safeState.serviceClients} services={safeState.serviceItems} appointments={safeState.serviceAppointments} contacts={safeState.contacts} accounts={safeState.accounts} onSaveClient={(c) => api.saveServiceClient(c).then(refreshData)} onDeleteClient={(id) => api.deleteServiceClient(id).then(refreshData)} onSaveService={(s) => api.saveCatalogItem(s).then(refreshData)} onDeleteService={(id) => api.deleteCatalogItem(id).then(refreshData)} onSaveAppointment={(a) => api.saveAppointment(a).then(refreshData)} onDeleteAppointment={(id) => api.deleteAppointment(id).then(refreshData)} />;

      case 'SYS_BRANCHES': return <BranchesView branches={safeState.branches} onSaveBranch={(b) => api.savePJEntity('branch', b).then(refreshData)} onDeleteBranch={(id) => api.deletePJEntity('branch', id).then(refreshData)} onManageSchedule={(b) => setCurrentView('SRV_BRANCH_SCHEDULE')} />;
      case 'SRV_BRANCH_SCHEDULE':
          ...
          return <BranchScheduleView branch={activeBranch} appointments={safeState.serviceAppointments} clients={safeState.serviceClients} onSaveAppointment={(a) => api.saveAppointment(a).then(refreshData)} onDeleteAppointment={(id) => api.deleteAppointment(id).then(refreshData)} onBack={() => setCurrentView('SYS_BRANCHES')} />;
      ...
      case 'SYS_CHAT': return <ChatView currentUser={currentUser} socket={socketRef.current} />;
  ...
  if (!currentUser) {
      if (showAuth) return <div className="relative min-h-screen bg-gray-50"><button onClick={() => setShowAuth(false)} className="fixed top-6 left-6 z-50 bg-white p-2 rounded-full shadow-lg border border-gray-100 text-gray-400 hover:text-indigo-600 transition-all"><ArrowLeft className="w-6 h-6" /></button><Auth onLoginSuccess={handleLoginSuccess} initialMode={authInitialMode} /></div>;
      return <LandingPage onGetStarted={() => { setAuthInitialMode('REGISTER'); setShowAuth(true); }} onLogin={() => { setAuthInitialMode('LOGIN'); setShowAuth(true); }} />;
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-inter text-gray-900 overflow-hidden">
      <HelpProvider currentView={currentView} onChangeView={setCurrentView}>
        <Sidebar currentView={currentView} onChangeView={setCurrentView} currentUser={currentUser} onUserUpdate={setCurrentUser} isMobileOpen={isMobileMenuOpen} setIsMobileOpen={setIsMobileMenuOpen} />
        
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
            <div className="md:hidden bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between z-40">
                <div className="flex items-center gap-2"><div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-black shadow-lg">F</div><span className="font-black text-sm text-gray-800 tracking-tighter">FinManager</span></div>
            ...
            <div className={`text-[10px] font-black uppercase tracking-widest px-4 py-1 flex items-center justify-center gap-2 transition-all ${syncStatus === 'offline' ? 'bg-rose-50 text-white' : syncStatus === 'syncing' ? 'bg-indigo-600 text-white' : 'bg-emerald-500 text-white'}`}>
            ...
            <div className="flex-1 overflow-y-auto relative scroll-smooth"><div className="p-4 md:p-8 max-w-[1600px] mx-auto">{renderContent()}</div></div>
        </main>
        {/* Chat Flutuante disponível em todas as telas autenticadas */}
        <ChatFloating currentUser={currentUser} socket={socketRef.current} />
      </HelpProvider>
    </div>
  );
};
