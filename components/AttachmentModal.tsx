...
import { X, FilePlus, Trash2, Download, Eye, FileText, Image as ImageIcon } from 'lucide-react';
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-scale-up border border-slate-100 flex flex-col max-h-[85vh]">
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
                ...
                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mt-1">Total: {urls.length} arquivo(s)</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-4">
            {urls.length === 0 ? (
                <div className="py-12 text-center">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Adicione notas fiscais ou comprovantes de pagamento.</p>
                </div>
            ) : (
                            ...
                            <div key={idx} className="group flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-2xl hover:border-indigo-200 transition-all">
                                <div className="flex items-center gap-4 overflow-hidden">
                                    ...
                                    <div className="overflow-hidden">
                                        <p className="text-sm font-bold text-slate-700 truncate">Anexo #{idx + 1}</p>
                                        <p className="text-[10px] text-slate-400 uppercase font-black tracking-tighter">Arquivo Digital</p>
                                    </div>
                                </div>
        ...
        <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row gap-4">
            ...
            <button 
                ...
                className="flex-1 flex items-center justify-center gap-2 py-4 bg-white border-2 border-indigo-600 text-indigo-600 rounded-2xl font-black text-sm hover:bg-indigo-50 transition-all shadow-sm"
            >
                <FilePlus className="w-5 h-5" /> Adicionar Arquivos
            </button>
            <button 
                onClick={onClose}
                className="sm:w-32 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-black transition-all shadow-xl"
            >
                Concluir
            </button>
        </div>
