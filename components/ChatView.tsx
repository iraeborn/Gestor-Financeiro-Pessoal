...
const ChatView: React.FC<ChatViewProps> = ({ currentUser, socket }) => {
    ...
    // Estado para rastrear contagem de não lidas por remetente
    ...
    // Limpa unreads ao selecionar um chat
    useEffect(() => {
        const key = selectedUser ? selectedUser.id : 'GENERAL';
        if (unreadCounts[key]) {
    ...
    useEffect(() => {
        ...
        const handleNewMessage = (msg: ChatMessage) => {
            ...
            // Lógica de Notificação e Badges
            const isFromSelected = selectedUser 
                ? (msg.senderId === selectedUser.id || (msg.senderId === currentUser.id && msg.receiverId === selectedUser.id))
                : (!msg.receiverId);

            if (!isFromSelected && msg.senderId !== currentUser.id) {
                // Notifica no badge lateral
                const key = msg.receiverId ? msg.senderId : 'GENERAL';
                setUnreadCounts(prev => ({
                    ...prev,
                    [key]: (prev[key] || 0) + 1
                }));
    ...
    }, [socket, currentUser.familyId, currentUser.id, selectedUser]);
            ...
            <div className={`w-full md:w-80 border-r border-gray-50 flex flex-col bg-slate-50/50 absolute md:relative inset-0 z-20 transition-transform duration-300 ${isListView ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    ...
                    <button 
                        ...
                        className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all border group relative ${selectedUser === null && !isListView ? 'bg-white border-indigo-200 shadow-md scale-[1.02]' : 'bg-transparent border-transparent hover:bg-gray-100'}`}
                    >
                        ...
                        <div className="text-left overflow-hidden flex-1">
                            ...
                            <p className="text-[10px] text-indigo-500 font-bold uppercase truncate">Canal Coletivo</p>
                        </div>
                        {unreadCounts['GENERAL'] > 0 && (
                            <span className="bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-sm animate-bounce">
                                {unreadCounts['GENERAL']}
                            </span>
                        )}
                    </button>
                    
                    <div className="pt-6 pb-2 px-2"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Contatos Privados</p></div>
                    ...
                    {members.filter(m => m.id !== currentUser.id).map(m => {
                        ...
                        return (
                            <button 
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all border relative ${selectedUser?.id === m.id ? 'bg-white border-indigo-200 shadow-md scale-[1.02]' : 'bg-transparent border-transparent hover:bg-gray-100'}`}
                            >
                                ...
                                {unread > 0 && (
                                    <span className="bg-indigo-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-md animate-pulse">
                                        {unread}
                                    </span>
                                )}
                            </button>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-black uppercase tracking-tighter flex items-center gap-1">
                                {selectedUser 
                                    ? (onlineUsers.has(selectedUser.id) ? <span className="text-emerald-500">Privado • Online</span> : <span className="text-gray-400">Privado • Offline</span>)
                                    : <span className="text-indigo-500">Público • Chat Coletivo</span>
                                }
                            </p>
                        </div>
                    </div>
                        ...
                        return (
                            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                                <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} max-w-[85%] md:max-w-[70%]`}>
                                    {!isMine && !selectedUser && <span className="text-[9px] font-black text-indigo-400 uppercase mb-1 ml-2 tracking-tighter">{msg.senderName}</span>}
                ...
                <div className="p-4 md:p-6 bg-white border-t border-gray-100">
                    <form onSubmit={handleSend} className="flex items-center gap-2 md:gap-4 bg-gray-50 p-2 rounded-[2rem] border border-gray-100 focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                        ...
                        <input 
                            ...
                            placeholder={selectedUser ? `Responder para ${selectedUser.name.split(' ')[0]}...` : "Enviar para o grupo..."}
