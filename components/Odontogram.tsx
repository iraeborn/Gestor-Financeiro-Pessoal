...
const Odontogram: React.FC<OdontogramProps> = ({ states, onToothClick }) => {
  ...
  return (
    <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-8 overflow-x-auto">
      <div className="flex flex-col items-center gap-2">
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Arcada Superior</span>
        <div className="flex gap-1.5">{adultTeethUpper.map(renderTooth)}</div>
      </div>
      
      <div className="h-px bg-slate-200 w-full"></div>

      <div className="flex flex-col items-center gap-2">
        <div className="flex gap-1.5">{adultTeethLower.map(renderTooth)}</div>
