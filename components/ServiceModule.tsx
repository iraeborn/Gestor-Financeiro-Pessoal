                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-gray-50 text-gray-500 font-medium">
                                <tr>
                                    ...
                                    <th className="px-6 py-3">Observações / Histórico</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredClients.map(c => (
                                    <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 font-medium text-gray-900">{c.contactName}</td>
                                        <td className="px-6 py-4 text-gray-500">{c.birthDate ? new Date(c.birthDate).toLocaleDateString('pt-BR') : '-'}</td>
                                        <td className="px-6 py-4 text-gray-500 truncate max-w-xs">{c.notes || '-'}</td>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                        ...
                        <form onSubmit={handleSaveClient} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 mb-1">Histórico / Observações</label>
                                <textarea className="w-full border rounded-lg p-2 text-sm" rows={3} value={clientForm.notes || ''} onChange={e => setClientForm({...clientForm, notes: e.target.value})} placeholder="Alergias, histórico, detalhes..." />
                            </div>
                            <div className="flex justify-end gap-2 mt-4">
                                <button type="button" onClick={() => setClientModalOpen(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
