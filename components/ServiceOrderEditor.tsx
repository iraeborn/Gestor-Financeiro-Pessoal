...
const ServiceOrderEditor: React.FC<ServiceOrderEditorProps> = ({ initialData, contacts, serviceItems, opticalRxs, branches, settings, onSave, onCancel }) => {
    ...
    const pricing = useMemo(() => {
        ...
        const total = items.reduce((sum, i) => sum + (i.isBillable !== false ? (Number(i.totalPrice) || 0) : 0), 0);
        return { total };
    }, [formData.items]);
                        ...
                        <div className="space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    <label className="block text-[10px] font-black uppercase text-gray-400 mb-2 ml-1">Título da OS</label>
                                    <input 
                                        type="text" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase text-gray-400 mb-2 ml-1">Técnico Responsável</label>
                                    <div className="relative">
                                        <UserCog className="w-4 h-4 text-gray-400 absolute left-4 top-4" />
                                        <select 
                                            value={formData.assigneeId || ''} 
                                            onChange={e => setFormData({...formData, assigneeId: e.target.value})}
                                            className="w-full pl-11 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold outline-none appearance-none"
                                        >
                            ...
                            <div className="space-y-4 pt-6 border-t border-gray-100">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><Package className="w-4 h-4 text-indigo-500" /> Itens e Peças</h3>
                                    <div className="flex gap-2">
                                        <select 
                                            ...
                                            onChange={e => { if(e.target.value) handleAddOSItem(e.target.value); e.target.value = ''; }}
                                        >
                                            <option value="">+ Adicionar do Catálogo</option>
                                            {serviceItems.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}
                                        </select>
                                        <button type="button" onClick={() => handleAddOSItem()} className="text-[10px] font-black uppercase text-gray-400 bg-gray-50 px-4 py-2 rounded-xl border border-gray-200">+ Manual</button>
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-50 rounded-[2rem] border border-gray-100 overflow-hidden shadow-inner overflow-x-auto">
                                    <table className="w-full text-left min-w-[600px]">
                                        <thead className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100">
                                            <tr>
                                                <th className="p-5">Descrição do Item</th>
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                                ...
                                                <tr key={item.id} className="bg-white/50 hover:bg-white transition-colors">
                                                    <td className="p-4">
                                                        <input 
                                                            ...
                                                            onChange={e => handleUpdateOSItem(item.id, 'description', e.target.value)}
                                                    ...
                                                    <td className="p-4">
                                                        <input 
                                                            type="number" 
                                                            value={item.quantity} 
                                                            onChange={e => handleUpdateOSItem(item.id, 'quantity', Number(e.target.value))}
                                                    ...
                                                    <td className="p-4">
                                                        <input 
                                                            ...
                                                            onChange={e => handleUpdateOSItem(item.id, 'unitPrice', Number(e.target.value))}
                                            ...
                                            {(formData.items || []).length === 0 && (
                                                <tr><td colSpan={5} className="p-10 text-center text-gray-400 text-xs italic">Nenhum item adicionado ainda.</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            <div className="space-y-4 pt-6 border-t border-gray-100">
                                <label className="block text-[10px] font-black uppercase text-gray-400 mb-2 ml-1">Observações Internas / Laudo Técnico</label>
                                <textarea 
                                    value={formData.description || ''} 
                                    onChange={e => setFormData({...formData, description: e.target.value})}
                                    className="w-full bg-gray-50 border-none rounded-[2rem] p-6 text-sm font-medium min-h-[150px] outline-none shadow-inner"
                                    placeholder="Descreva detalhes do defeito, solução ou peças trocadas..."
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-gray-100 p-8 shadow-sm space-y-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="PRONTA">Pronta p/ Entrega</option>
                            </select>
                        </div>

                        <div className="space-y-4">
                            <label className="block text-[10px] font-black uppercase text-gray-400 mb-2 ml-1">Prioridade</label>
                            <div className="grid grid-cols-2 gap-2">
                                {['BAIXA', 'MEDIA', 'ALTA', 'URGENTE'].map(p => (
                                    <button 
                                        key={p} 
                                        type="button"
                                        onClick={() => setFormData({...formData, priority: p as OSPriority})}
                                        className={`py-2 rounded-xl text-[10px] font-black uppercase border-2 transition-all ${formData.priority === p ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-50 text-gray-400 hover:bg-gray-50'}`}
                                    >
                                        {p}
                                    </button>
                                ))}
                        ...
                        <div className="pt-6 border-t border-gray-100">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Valor Final</p>
                            <p className="text-4xl font-black text-gray-900">{formatCurrency(pricing.total)}</p>
                        </div>
                        ...
                        {settings?.activeModules?.optical && (
                            <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100 space-y-4">
                                <div className="flex items-center gap-2 text-indigo-700 font-black text-[10px] uppercase tracking-widest">
                                    <Glasses className="w-4 h-4" /> Ótica / Receita
                                </div>
                                <select 
                                    value={formData.rxId || ''} 
...
export default ServiceOrderEditor;
