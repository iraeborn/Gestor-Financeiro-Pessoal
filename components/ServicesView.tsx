...
const ServicesView: React.FC<ServicesViewProps> = ({ 
    ...
    const shouldSyncWithCatalog = (status: string) => {
        return isCatalog; // Apenas o catálogo deve ser 100% dinâmico. Registros de venda devem manter preços manuais se existirem.
    };
    ...
    const getResolvedPrice = (item: any): number => {
        ...
        // Se o registro já tem itens com preços, usamos eles. 
        // A resolução dinâmica do catálogo só acontece em tempo de composição no modal.
        ...
        return items.map(item => {
            if (item.serviceItemId) {
                ...
                if (latest) {
                    ...
                    // Lógica corrigida: Se o item já tem um unitPrice (veio do banco), mantemos ele.
                    // Só pegamos do catálogo se for um item novo ou se estivermos explicitamente no modo catálogo.
                    const catalogPrice = getResolvedPrice(latest);
                    const unitPriceToUse = (item.unitPrice !== undefined && item.unitPrice !== null) 
