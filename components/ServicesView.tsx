...
import { Wrench, ShoppingBag, FileSignature, FileText, Plus, Search, Trash2, CheckCircle, Clock, X, DollarSign, Calendar, Filter, Box, Tag, Percent, BarChart, AlertTriangle, ArrowRight, TrendingUp, ScanBarcode, Loader2, Globe, Image as ImageIcon, Calculator, ReceiptText, UserCircle, User, Package, Zap, Info, UserCheck, Timer, Layers, ListChecks, RefreshCw } from 'lucide-react';
...
const ServicesView: React.FC<ServicesViewProps> = ({ 
    ...
    const resolveItems = (items: any[]) => {
        return items.map(item => {
            ...
            return { ...item, isFromCatalog: false };
        });
    };
                    ...
                    <div key={item.id} className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all group">
                         <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex flex-col items-end gap-1">
                                {isCatalog ? (
                                    <>
                                        <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${item.type === 'SERVICE' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>
                                            {item.type === 'SERVICE' ? 'SERVIÇO' : 'PRODUTO'}
                                        </span>
                                        <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${item.isComposite ? 'bg-purple-50 text-purple-600' : 'bg-slate-100 text-slate-500'}`}>
                                            {item.isComposite ? 'COMPOSTO' : 'SIMPLES'}
                                        </span>
                                    </>
                                ) : (
                                        ...
                                        <div className="bg-slate-50 rounded-2xl border border-gray-200 overflow-hidden shadow-inner overflow-x-auto">
                                            <table className="w-full text-left border-collapse min-w-[800px]">
                                                ...
                                                <tbody className="divide-y divide-gray-200">
                                                    {pricing.resolvedList.map((item: any) => (
                                                        <tr key={item.id} className="bg-white hover:bg-indigo-50/10 transition-colors">
                                                            <td className="px-4 py-3">
                                                                <div className="flex flex-col gap-1">
                                                                    <div className="flex items-center gap-1.5">
                                                                        <input type="text" value={item.code} readOnly={item.isFromCatalog} onChange={e => handleUpdateOSItem(item.id, 'code', e.target.value)} className={`w-full bg-transparent border-none focus:ring-0 text-[10px] font-black placeholder-indigo-200 ${item.isFromCatalog ? 'text-indigo-600' : 'text-indigo-400'}`} placeholder="Código"/>
                                                                        {item.isFromCatalog && <span title="Vínculo automático com o catálogo" className="text-indigo-500"><RefreshCw className="w-2.5 h-2.5 animate-spin-slow" /></span>}
                                                                    </div>
                                                                    <input type="text" value={item.description} readOnly={item.isFromCatalog} onChange={e => handleUpdateOSItem(item.id, 'description', e.target.value)} className={`w-full bg-transparent border-none focus:ring-0 text-xs font-bold ${item.isFromCatalog ? 'text-gray-900' : 'text-gray-500'}`} placeholder="Descrição do item..."/>
                                                                </div>
                                                            </td>
                                                            ...
                                                            <td className="px-2 py-3"><input type="number" step="0.01" value={item.unitPrice} readOnly={item.isFromCatalog} onChange={e => handleUpdateOSItem(item.id, 'unitPrice', Number(e.target.value))} className={`w-full bg-transparent border-none focus:ring-0 text-xs font-black text-center ${item.isFromCatalog ? 'text-indigo-700' : ''}`}/></td>
                                                            {isOS && (
                                                                <>
                                                            ...
                                                            <td className={`px-2 py-3 text-xs font-black text-right ${item.isFromCatalog ? 'text-indigo-800' : 'text-gray-900'}`}>{formatCurrency(item.totalPrice)}</td>
                                                    ...
                                                    {pricing.resolvedList.length === 0 && (<tr><td colSpan={isOS ? 10 : 5} className="px-4 py-12 text-center text-gray-400 text-xs italic">Clique em catálogo ou manual para adicionar itens.</td></tr>)}
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
