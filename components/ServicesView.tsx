...
const ServicesView: React.FC<ServicesViewProps> = ({ 
    ...
    // Impostos estimados (ex: 6% Simples)
    ...
    const pricing = useMemo(() => {
        ...
        // Encontrar item do catálogo se houver
    ...
    useEffect(() => {
        if (pricing && (currentView === 'SRV_SALES' || currentView === 'SRV_PURCHASES')) {
            setFormData(prev => ({ ...prev, amount: pricing.net }));
        }
    }, [pricing?.net]);
    ...
    const handleOpenModal = (item?: any) => {
        ...
        if (currentView === 'SRV_CATALOG') {
            ...
            if (item) {
                setFormData({ ...item, grossAmount: item.gross_amount || item.amount, discountAmount: item.discount_amount || 0, taxAmount: item.tax_amount || 0 });
                // Tenta inferir taxPercent
                const base = (item.gross_amount || item.amount) - (item.discount_amount || 0);
                if (base > 0) setTaxPercent(Math.round(((item.tax_amount || 0) / base) * 100));
            } else {
    ...
    const handleBarcodeScanned = async (code: string) => {
        ...
        if (code.length > 7) {
            ...
            try {
                ...
                if (existing) {
                    const confirm = await showConfirm({ title: 'Produto Existente', message: `O produto "${existing.name}" já está no catálogo. Carregar dados?`, confirmText: 'Carregar' });
                    if (confirm) { setFormData({ ...existing }); setProductSource('Catálogo Local'); }
                } else {
                    const res = await fetch(`https://world.openfoodfacts.org/api/v2/product/${code}`);
                    const data = await res.json();
                    if (data.status === 1 && data.product) {
                        const p = data.product;
                        setFormData((prev: any) => ({ ...prev, name: p.product_name || prev.name, brand: p.brands || '', unit: 'UN', imageUrl: p.image_url || p.image_front_url || '' }));
                        setProductSource('Open Food Facts');
                    }
                }
            } catch (e) { showAlert("Erro ao buscar info.", "error"); } finally { setLoadingProductInfo(false); }
        }
    };
    ...
    const handleDelete = async (id: string) => {
        if (await showConfirm({ title: 'Excluir Item', message: 'Tem certeza?', variant: 'danger' })) {
    ...
    const handleGenerateTransaction = async (item: any) => {
        if (await showConfirm({ title: 'Gerar Financeiro', message: 'Criar lançamento financeiro para este item?', confirmText: 'Sim, Gerar' })) {
            let trans: any = {
                description: item.description || item.title || 'Venda',
                amount: item.amount,
                    ...
                    <div key={item.id} className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:border-indigo-300 transition-all group">
                        ...
                        <div className="bg-gray-50 rounded-xl p-3 mb-4 space-y-1">
                            <div className="flex justify-between text-xs">
                                ...
                                <span className="text-gray-700">{formatCurrency(item.gross_amount || item.amount)}</span>
                            </div>
                        ...
                        <div className="flex justify-between items-center text-xs text-gray-400">
                             ...
                             <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                {(item.status === 'DRAFT' || item.status === 'APPROVED') && (
                                    <button onClick={() => handleGenerateTransaction(item)} className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-colors" title="Faturar / Gerar Financeiro">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    <div className="relative flex-1 md:w-64">
                        ...
                        <input type="text" placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 border rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                    </div>
                    <button onClick={() => handleOpenModal()} className="bg-indigo-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-100">
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl p-8 max-h-[95vh] overflow-y-auto animate-scale-up">
                        ...
                        <form onSubmit={handleSave} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <select className="w-full border rounded-xl p-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-500" value={formData.catalogItemId || ''} onChange={e => handleSelectItem(e.target.value)}>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" placeholder="Nome da venda/compra" className="w-full border rounded-xl p-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} required />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Cliente / Fornecedor</label>
                                            <select className="w-full border rounded-xl p-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-500" value={formData.contactId || ''} onChange={e => setFormData({...formData, contactId: e.target.value})} required>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input type="date" className="w-full border rounded-xl p-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={formData.date || ''} onChange={e => setFormData({...formData, date: e.target.value})} required />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Status</label>
                                                <select className="w-full border rounded-xl p-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-500" value={formData.status || 'DRAFT'} onChange={e => setFormData({...formData, status: e.target.value})}>
                                                    <option value="DRAFT">Orçamento (Rascunho)</option>
                                                    <option value="APPROVED">Aprovado pelo Cliente</option>
                                                    <option value="CONFIRMED">Vendido (Faturar)</option>
                                    ...
                                    <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200 flex flex-col justify-between">
                                        <div className="space-y-4">
                                            <h3 className="font-bold text-gray-700 flex items-center gap-2 border-b border-gray-200 pb-2"><ReceiptText className="w-4 h-4"/> Simulador de Preço</h3>
                                            
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Valor Bruto (Base)</label>
                                                <div className="relative">
                                                    <span className="absolute left-3 top-2.5 text-gray-400 text-sm">R$</span>
                                                    <input type="number" step="0.01" className="w-full pl-9 border rounded-xl p-2 font-bold text-gray-800" value={formData.grossAmount} onChange={e => setFormData({...formData, grossAmount: e.target.value})} />
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <input type="number" step="0.01" className="w-full border rounded-xl p-2 text-sm text-rose-600 font-medium" value={formData.discountAmount} onChange={e => setFormData({...formData, discountAmount: e.target.value})} />
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Impostos (%)</label>
                                                    <input type="number" className="w-full border rounded-xl p-2 text-sm text-gray-600" value={taxPercent} onChange={e => setTaxPercent(Number(e.target.value))} />
                                                </div>
                                            </div>
                                        ...
                                        {pricing && pricing.cost > 0 && (
                                            <div className={`mt-6 p-3 rounded-xl border ${pricing.margin > 30 ? 'bg-emerald-50 border-emerald-100' : 'bg-amber-50 border-amber-100'}`}>
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className="text-[10px] font-bold uppercase text-gray-500">Lucro Estimado</span>
                                                    <span className={`text-xs font-bold ${pricing.margin > 30 ? 'text-emerald-700' : 'text-amber-700'}`}>{Math.round(pricing.margin)}%</span>
                                                </div>
                                                <p className="font-bold text-gray-800">{formatCurrency(pricing.profit)}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {/* Os outros formulários (OS, NF, Contratos) permanecem similares, simplificados para brevidade aqui */}
                            {currentView === 'SRV_CATALOG' && (
                                <div className="space-y-4">
                                    ...
                                    <input type="text" placeholder="Nome do Item" className="w-full border rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 outline-none" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} required />
                                    <div className="grid grid-cols-2 gap-4">
                                        <input type="number" step="0.01" placeholder="Preço de Venda" className="border rounded-xl p-3" value={formData.defaultPrice} onChange={e => setFormData({...formData, defaultPrice: e.target.value})} required />
                                        <input type="number" step="0.01" placeholder="Custo (Opcional)" className="border rounded-xl p-3" value={formData.costPrice} onChange={e => setFormData({...formData, costPrice: e.target.value})} />
                                    </div>
                                    <textarea placeholder="Descrição" className="w-full border rounded-xl p-3" rows={3} value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} />
                                </div>
                            )}

                            {currentView === 'SRV_OS' && (
                                <div className="space-y-4">
                                    <input type="text" placeholder="Título da OS" className="w-full border rounded-xl p-3" value={formData.title || ''} onChange={e => setFormData({...formData, title: e.target.value})} required />
                                    <div className="grid grid-cols-2 gap-4">
                                        <input type="number" placeholder="Valor Total" className="border rounded-xl p-3" value={formData.totalAmount || ''} onChange={e => setFormData({...formData, totalAmount: e.target.value})} />
                                        <select className="border rounded-xl p-3 bg-white" value={formData.status || 'OPEN'} onChange={e => setFormData({...formData, status: e.target.value})}>
                                    ...
                                    <select className="w-full border rounded-xl p-3 bg-white" value={formData.contactId || ''} onChange={e => setFormData({...formData, contactId: e.target.value})} required>
                            ...
                            {/* ... (Os demais formulários mantêm a lógica básica de salvar) */}

                            <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 text-gray-500 hover:bg-gray-100 rounded-xl font-bold transition-colors">Cancelar</button>
                                <button type="submit" className="bg-indigo-600 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-colors">Salvar {header.label}</button>
                            </div>
                        </form>
                    </div>
                </div>
