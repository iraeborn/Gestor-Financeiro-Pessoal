...
import { ServiceOrder, CommercialOrder, Contract, Invoice, Contact, ViewMode, TransactionType, TransactionStatus, ServiceItem, OSItem, Category, Account, CompanyProfile, TaxRegime, OSStatus, OSType, OSOrigin, OSPriority, KanbanItem, KanbanColumnConfig, Member } from '../types';
import { Wrench, ShoppingBag, FileSignature, FileText, Plus, Search, Trash2, CheckCircle, Clock, X, DollarSign, Calendar, Filter, Box, Tag, Percent, BarChart, AlertTriangle, ArrowRight, TrendingUp, ScanBarcode, Loader2, Globe, Image as ImageIcon, Calculator, ReceiptText, UserCircle, User, Package, Zap, Info, UserCheck, Timer, Layers, ListChecks, RefreshCw, Share2, Send, MessageSquare, FileUp, Download, Monitor, FileSearch, Link2, LayoutGrid, LayoutList, Trello, UserCog, Pencil } from 'lucide-react';
...
const ServicesView: React.FC<ServicesViewProps> = ({ 
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        // Find assignee name from members list
        ...
        const common = { id, contactId: finalContactId, contactName: contactSearch, assigneeId: formData.assigneeId, assigneeName };
    ...
    const renderGridContent = () => {
        ...
        if (isSales && viewType === 'KANBAN') {
            const kanbanItems: KanbanItem[] = filtered.map(o => ({
                id: o.id,
                title: o.description,
                subtitle: o.contactName || 'Sem cliente',
                status: o.status,
                amount: o.amount,
                date: o.date,
                tags: [o.type],
            ...
            return (
                <div className="w-full h-full overflow-hidden">
                    <KanbanBoard items={kanbanItems} columns={SALE_KANBAN_COLUMNS} onItemClick={handleOpenModal} onStatusChange={handleSaleStatusUpdate} />
                </div>
            );
        }
                            ...
                            <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all group overflow-hidden flex flex-col">
                                <div className="p-5 flex-1">
                                    ...
                                    <div className="space-y-2 mb-4">
                                        ...
                                        {item.assigneeName && (
                                            <div className="flex items-center gap-2 text-[10px] text-emerald-600 font-bold bg-emerald-50 w-fit px-2 py-0.5 rounded uppercase tracking-tighter">
                                        ...
                                        {!isOrder && (
                                            <div className="flex items-center justify-between text-[10px] uppercase font-bold">
                                                <div className="flex items-center gap-1 text-gray-400"><Zap className={`w-3 h-3 ${getOSPriorityColor(item.priority)}`} /> Prioridade: <span className={getOSPriorityColor(item.priority)}>{item.priority}</span></div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                ...
                {filtered.map(item => {
                    ...
                    return (
                        <div key={item.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all group flex flex-col h-full">
                            ...
                            {item.assigneeName && (
                                <div className="mb-4 text-[10px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded w-fit uppercase">
                                ...
                                <div className="lg:col-span-3 space-y-6">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <select 
                                                            ...
                                                            className="w-full pl-9 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-bold bg-white appearance-none cursor-pointer"
                                                        >
                                    ...
                                    {isNF && (
                                        <div className="space-y-4">
                                            ...
                                            <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in">
                                                ...
                                                <button 
                                                    ...
                                                    className="bg-white text-emerald-700 px-5 py-2.5 rounded-lg flex items-center gap-2 text-[10px] font-black uppercase tracking-widest hover:bg-emerald-50 transition-all border border-emerald-100 shadow-sm whitespace-nowrap"
                                                >
