  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 flex items-center justify-between flex-shrink-0">
            ...
            <button onClick={onOpenNotifications} className="relative p-2 text-gray-400 hover:text-indigo-600 rounded-xl transition-all">
                        ...
                        <div className="space-y-1">
                            {section.items.map((item: any) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${currentView === item.id ? 'bg-indigo-50 text-indigo-700 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                                >
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50">
             <button onClick={() => setIsWorkspaceDropdownOpen(!isWorkspaceDropdownOpen)} className="flex items-center gap-3 w-full p-2 hover:bg-white rounded-xl transition-all border border-transparent hover:border-gray-200 text-left">
             ...
             {isWorkspaceDropdownOpen && (
                 <div className="absolute bottom-16 left-4 right-4 bg-white rounded-xl shadow-xl border border-gray-100 p-2 z-50">
                     <button onClick={() => {setIsProfileModalOpen(true); setIsWorkspaceDropdownOpen(false);}} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 rounded-lg flex items-center gap-2"><UserCog className="w-4 h-4" /> Perfil</button>
                     <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 rounded-lg flex items-center gap-2"><LogOut className="w-4 h-4" /> Sair</button>
                 </div>
             )}
        </div>
